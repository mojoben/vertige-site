...
import { fetchPortalProperties, fetchPortalAvailability, fetchPortalContent, chaletPath, type PortalContent } from '@/lib/portal-client'
...
const LOC_ICONS: Record<string, React.ReactNode> = {
  ...
  village: <><path d="M9 4L3 6v14l6-2 6 2 6-2V4l-6 2z" /><path d="M9 4v14M15 6v14" /></>,
  ...
  transfers: <><path d="M2 7h12l6 4v5h-2.5M2 7v9h2.5M2 12h18" /><circle cx="7" cy="16.5" r="1.8" /><circle cx="16.5" cy="16.5" r="1.8" /><path d="M9 16.5h5" /></>,
...
export async function generateMetadata({ params }: { params: Promise<{ country: string; resort: string; slug: string }> }) {
  const { slug } = await params
  const { properties, live } = await fetchPortalProperties()
  const p = live ? properties.find((x) => x.slug === slug) : undefined
  if (!p) return { title: 'Luxury Ski Chalet — Vertige' }
  const path = chaletPath(p)
  return {
    // Chalet + resort in the title is worth more than either alone (SEO).
    title: `${p.name}, ${p.resort} — Luxury Ski Chalet | Vertige`,
    description: `${p.name} in ${p.resort}, ${p.country} — sleeps ${p.sleeps} across ${p.bedrooms} bedrooms. A hand-picked Vertige chalet with full concierge.`,
    alternates: { canonical: `${SITE_ORIGIN}${path}` },
  }
}

export default async function ChaletDetailPage({ params }: { params: Promise<{ country: string; resort: string; slug: string }> }) {
  const { country, resort, slug } = await params

  const { properties, live } = await fetchPortalProperties()
  const p = live ? properties.find((x) => x.slug === slug) : undefined
  // One canonical URL per chalet: a live chalet reached under the wrong
  // destination 308s to its true path; unknown slugs are 404s (the sample
  // renders only when the portal feed is down — dev fallback).
  if (p) {
    const canonical = chaletPath(p)
    if (canonical !== `/${country}/${resort}/${slug}`) permanentRedirect(canonical)
  } else if (live) {
    notFound()
  }
  ...
  const crumbs = p
    ? [
        ...
        { name: c.country, item: `${SITE_ORIGIN}/${country}` },
        { name: c.resort, item: `${SITE_ORIGIN}/${country}/${resort}` },
        { name: c.name, item: `${SITE_ORIGIN}${chaletPath(p)}` },
      ]
    : null
        ...
        <div className="acards">
          {(live && properties.length > 1
            ? (() => {
                ...
                href: chaletPath(x), img: x.images[0]?.url ?? '/images/chalets/ext-02.webp',
